...
function Transition:draw()
    if self.isActive or (self.direction == "out" and self.elapsed < self.duration) then
        ...
        elseif self.transitionType == "circular" then
            ...
            -- Distance to each corner from the center
            local distToTopLeft = math.sqrt(centerX^2 + centerY^2)
            local distToTopRight = math.sqrt((screenWidth - centerX)^2 + centerY^2)
            local distToBottomLeft = math.sqrt(centerX^2 + (screenHeight - centerY)^2)
            local distToBottomRight = math.sqrt((screenWidth - centerX)^2 + (screenHeight - centerY)^2)
            
            -- Use the maximum distance to ensure the circle covers the entire screen
            local maxRadius = math.max(distToTopLeft, distToTopRight, distToBottomLeft, distToBottomRight)
            
            -- Add extra padding to ensure full coverage even at corners
